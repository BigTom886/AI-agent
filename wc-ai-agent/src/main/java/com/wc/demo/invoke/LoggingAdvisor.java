 ...
 * - 在请求发送给模型之前，可以查看/修改请求（如添加日志、修改提示词）
 * - 在模型返回响应之后，可以查看/修改响应（如记录耗时、过滤敏感词）
 *
 * Spring AI 1.8.9 提供两个 Advisor 接口：
 * - CallAdvisor —— 拦截同步调用（chatClient.prompt().call()）
 * - StreamAdvisor —— 拦截流式调用（chatClient.prompt().stream()）
 *
...
public class LoggingAdvisor implements CallAdvisor, StreamAdvisor {
    ...
    /**
     * Advisor 的名称标识
     * 用于在日志和调试中识别当前 Advisor，建议返回有意义的名称
     *
     * @return Advisor 名称
     */
    @Override
    ...
    /**
     * Advisor 的执行优先级
     * 数值越小，优先级越高（越早拦截请求，越晚处理响应）
     * 例如：0 最高优先级，Integer.MAX_VALUE 最低优先级
     *
     * @return 优先级数值
     */
    @Override
     ...
     * 执行流程：
     * 1. 请求阶段：打印用户输入等请求信息
     * 2. 调用下一个 Advisor：streamAdvisorChain.nextStream(request)
     * 3. 响应阶段：打印模型返回的流式响应信息
     *
    ...
    public Flux<ChatClientResponse> adviseStream(ChatClientRequest chatClientRequest,
        ...
        // 请求阶段：记录用户输入
        System.out.println("[LoggingAdvisor-Stream] 请求: " + chatClientRequest.prompt().getUserMessages());
     ...
     * 执行流程：
     * 1. 请求阶段：打印用户输入等请求信息
     * 2. 调用下一个 Advisor：callAdvisorChain.nextCall(request)
     * 3. 响应阶段：打印模型返回的响应信息
     *
     * @param chatClientRequest 当前的请求对象，包含用户消息、系统提示词、Advisor 参数等
    ...
    public ChatClientResponse adviseCall(ChatClientRequest chatClientRequest, CallAdvisorChain callAdvisorChain) {
        // 请求阶段：记录用户输入
        System.out.println("[LoggingAdvisor-Call] 请求: " + chatClientRequest.prompt().getUserMessages());
        // 调用链的下一个节点（可能是另一个 Advisor，也可能是模型本身）
